...
namespace Sando.Indexer.Documents
{
	public class FieldDocument : SandoDocument
	{
        ...
        { 	
            ...
            fields.Add(new Field(SandoField.DataType.ToString(), fieldElement.FieldType, Field.Store.YES, Field.Index.ANALYZED));
            AddBodyField(fields, new Field(SandoField.Body.ToString(), fieldElement.RawSource, Field.Store.NO, Field.Index.ANALYZED));
            fields.Add(new Field(SandoField.ClassId.ToString(), fieldElement.ClassId.ToString(), Field.Store.YES, Field.Index.NOT_ANALYZED));
            fields.Add(new Field(SandoField.ClassName.ToString(), fieldElement.ClassName, Field.Store.YES, Field.Index.ANALYZED));
            fields.Add(new Field(SandoField.Modifiers.ToString(), fieldElement.Modifiers, Field.Store.YES, Field.Index.ANALYZED));
            fields.Add(new Field(SandoField.InitialValue.ToString(), fieldElement.InitialValue, Field.Store.YES, Field.Index.ANALYZED));
            return fields;
		}
        ...
        public override object[] GetParametersForConstructor(string name, ProgramElementType programElementType, string fullFilePath, int definitionLineNumber, int definitionColumnNumber, string snippet, Document document)
		{
			...
			string initialValue = document.GetField(SandoField.InitialValue.ToString()).StringValue();
